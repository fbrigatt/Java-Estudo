...
public class OlaProperties extends Application {
	...
	public void start(Stage s) throws Exception {
		...
		raiz.getChildren().add(new Separator(Orientation.HORIZONTAL));
		raiz.getChildren().add(new Label("Usando expressões de StringProperty"));
	...
	private static Node[] OlaExpressoesBind() {
		...
		Label lblNome = new Label();
		lblNome.textProperty().bind(txtNome.textProperty().concat(", ").concat(txtSaudacao.textProperty()));
	...
	private static Node[] olaExpressoesDoubleBind() {
		...
		lblResultado.textProperty().bind(sld1.valueProperty().add(sld2.valueProperty()).asString());
	...
	private static Node[] olaListeners() {
		String valores[] = { "Garrafa", "Copo", "Monitor", "Notebook", "Celular" };
		// na próxima falamos de FXCollections
		ComboBox<String> cmbValores = new ComboBox<String>(FXCollections.observableArrayList(valores));
